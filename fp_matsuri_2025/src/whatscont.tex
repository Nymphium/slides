...
\begin{frame}[t,fragile]
	...
	\begin{tikzpicture}
		\coordinate (p1);
		\coordinate[right=.313\linewidth of p1] (p2);
		\coordinate[below=4.4\zh of p2] (p3);
		\coordinate[left=.313\linewidth of p3] (p4);
		...
		\node[right=.3 of c2l]                (c2r)                                 {\mintinline{scala}|g(v1)|};

		\node[below=.2 of c2l]                (c3l) {\mintinline{scala}|val v3 =|};
		\node[right=.3 of c3l]                (c3r)                                 {\mintinline{scala}|h(v2)|};

		\node[below=.2 of c3l, xshift=-1\zw]  (c4)  {\mintinline{scala}|?$\dots\dots$?|\vphantom{l}};
	\end{tikzpicture}

	\pause

	\begin{tikzpicture}[overlay]
		\coordinate[right=7.36 of p1] (p1');
		\coordinate[right=.404\linewidth of p1'] (p2');
		\coordinate[below=4.4\zh of p2'] (p3');
		\coordinate[left=.404\linewidth of p3'] (p4');

		\path[draw=black, line width=1pt] (p1') -- (p2') -- (p3') -- (p4') -- (p1');

		\node[right=6.44 of c1r] (a1) {\mintinline{scala}#f()   |>#};
		\node[right=3 of c2r]    (a2) {\mintinline{scala}#((v1) => g(v1) |>#};
		\node[right=3 of c3r]    (a3) {\mintinline{scala}#((v2) => h(v2) |>#};
		\node[right=6.04 of c4]  (a4) {\mintinline{scala}#((v3) => ?$\dots\dots$?  )))#};
		...
		at ([xshift=1.1\zw, yshift=-1.35\zh]a4.south)
		{\mintinline[fontsize=\footnotesize]{scala}#def x |> k === k(x)#};

		\draw[boldarrow, draw=subhighlight, bend right = 25] ([yshift=-.4\zh]p3.south east) to ([yshift=-.4\zh]p4'.south west) node[below,xshift=-2.5\zw,yshift=-1\zh] {\scriptsize\it Continuation Passing Style};
