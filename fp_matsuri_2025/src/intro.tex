...
\begin{frame}
	\osietes
	\vskip-.8\baselineskip\relax

	\centerline{\includegraphics[height=3\baselineskip]{img/osietetitle.png}\raisebox{1.8\baselineskip}{\scriptsize\footnote{\url{https://arkw.net/products/web/hakase/}}}}
	\vspace*{-.7\zw}

	\hspace*{.80\textwidth}
	\includegraphics[width=4\zw]{img/osiete1.png}
	\vspace*{-5\zw}

	\begin{center}
		\begin{minipage}[t]{.815\textwidth}
			\begin{itemize}
				\item[\color{basetext}\Large\osietes Q.] What is the \underline{most expressive} concept of

				      \textcolor{subhighlight}{functional programming}?

				      \pause
				\item[\color{basetext}\Large\osietes A.] Definitely, it's a
			\end{itemize}
		\end{minipage}
	\end{center}
	\vspace*{-.4\zw}
	...
	\hspace{-.03\linewidth}
	\makebox[\linewidth]{{\Huge\scalefont{1.65}\bf\color{highlight}CONTINUATION!}\hspace*{-3\zw}\raisebox{-.5\baselineskip}{\color{basetext!80!}\tiny$\left[\textsf{要出典}\right]$}}
...
\begin{frame}[c]
	...
	% {\huge \boldslant The History of}

	{\huge \scalefont{1.7}\boldslant \textcolor{highlight}{Continuations:}}
...
\end{frame}
\switchfooter
\TableOfContents{sectionstyle=show,subsectionstyle=show}
...
\begin{frame}
	...
	\begin{itemize}[<+->]
		...
		\item[\emoji{man-raising-hand}] \only<.(2)>{\emoji{check-mark-button}\semibf} Yeah, I think they're callback functions.
...
\begin{frame}[fragile]
	...
	\begin{tikzpicture}[overlay, every node/.style={inner sep=0pt, outer sep=0pt}]
		\draw[boldarrow,draw=orange,bend left=30] ([yshift=-.4\zh]k1i.south east) to ([xshift=11\zw, yshift=-1\zh] c2r) node[right=2.2\zw, yshift=.7\zh] { \small with \mintinline{scala}|for| };
		...
		\draw<+(1)->[boldarrow,draw=orange,bend left=30]([yshift=-1\zh]c2l.west) to ([yshift=-.5\zh]c1l.south west) node[right=-1.7\zw,yshift=-3.15\zh] { \small desugar };
	\end{tikzpicture}
\end{frame}
...
\begin{frame}[fragile]
	...
	\begin{tikzpicture}[overlay]
		...
		\draw[boldarrow, draw=subhighlight, bend right = 25] ([yshift=-.4\zh]p3.south east) to ([yshift=-.4\zh]p4'.south west) node[below, xshift=-2.8\zw, yshift=-1.5\zh] (cps) {\scriptsize\it Pseudo Continuation-Passing Style Conversion};

		\node at ([yshift=-1\zw,xshift=2\zw]cps.south) {\rotatebox{-30}{\large\textcolor{highlight}{\emoji{exclamation-question-mark}}}};
	\end{tikzpicture}
