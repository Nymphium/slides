\section{Who talks}
\begin{frame}
  \frametitlesec

  \begin{figure}[ht]
    \begin{tikzpicture}
      \node (img) {\includegraphics[height=2\zh]{img/yh.png}};
      \node[draw, rectangle callout, right = .7 of img, callout absolute pointer={($(img.east)+(-.1,.2) $)}, yshift = 1\zh] {Hello, I'm bisho-jo};
    \end{tikzpicture}
  \end{figure}

  \vskip-\baselineskip

  \begin{itemize}
    \item[\emoji{office}] \itemheader{\href{https://kanmu.co.jp}{Kanmu,Inc.}}
    \item[\textcolor{gray}{\faicon{paperclip}}] {\small \href{https://twitter.com/Nymphium}{\textcolor{blue!60!white}{\faicon{twitter}}} \href{https://github.com/Nymphium}{\textcolor{black}{\faicon{github}}} Nymphium}
    \item[\emoji{japanese-symbol-for-beginner}] Golang 2nd-year student
    \item[\emoji{nerd-face}] Interesed in...
      {\small %
        \begin{itemize}
          \item Compiler optimization,
          \item Language specification,
          \item Programming language theory.
        \end{itemize}}
  \end{itemize}
\end{frame}

